...
module.exports = class DServer {
  constructor(dpki, myKeypair, port = 1000) {
    this.dpki = dpki;
    ...
    socketio.use(async (socket, next) => {
      ...
      const dpkiPublickey = await this.dpki.getIdentity(query.addr);
      if (dpkiPublickey.pubkey !== query.pubkey) {
        return next(new Error('PUBKEY MISMATCH'));
      }
