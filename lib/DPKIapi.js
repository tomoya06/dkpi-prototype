...
export default class DPKI {
    constructor(config) {
        ...
        this.DPKI = w3.eth.contract(contract.abi)
        this.dpkiInstance = DPKI.at(ethConfig.deploy.address)
    ...
    emitAddedIdentity() {
        contractFunctionFactory('emitAddedIdentity')
            .then(result => {
                console.log(`EMIT ADDEDIDENTITY`)
            })
    }

    addIdentity(ipAddr, pubKey) {
