...
比特币交易数据在打包到一个区块中后，交易就算初步确认了；当区块链接到前一个区块之后，交易会得到进一步的确认；在连续得到6个区块确认之后，这笔交易基本上就不可逆转地得到确认了。

#### 账户

一笔交易包含支付方和接受方，意味着涉及到两个账户。在比特币系统中申请账户的时候，会为每个账户分配一个独一无二的比特币地址，并生成一份私钥，私钥需要妥善保管，因为只有私钥是拥有该账户的唯一凭证，证明使用者对该比特币地址上的比特币拥有所有权。
